...
`~/.claude/pai/user/skillcustomizations/annual-reports/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:8888/notify \
     ...
     -d '{"message": "Running the WORKFLOWNAME workflow in the annual-reports skill to ACTION"}' \
     > /dev/null 2>&1 &
   ```

2. **Output text notification**:
   ```
   Running the **WorkflowName** workflow in the **annual-reports** skill to ACTION...
   ```

**This is not optional. Execute this curl command immediately upon skill invocation.**

# annual-reports - Security Report Aggregation
...
bun run ~/.claude/skills/security/annual-reports/tools/UpdateSources.ts

# List all sources
bun run ~/.claude/skills/security/annual-reports/tools/ListSources.ts [category]

# Fetch a specific report
bun run ~/.claude/skills/security/annual-reports/tools/FetchReport.ts <vendor> <report-name>
```
