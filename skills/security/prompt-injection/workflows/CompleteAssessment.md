...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the CompleteAssessment workflow in the prompt-injection skill to test security"}' \
  > /dev/null 2>&1 &
```

Running the **CompleteAssessment** workflow in the **prompt-injection** skill to test security...
