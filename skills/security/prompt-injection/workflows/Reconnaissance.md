...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the Reconnaissance workflow in the prompt-injection skill to map attack surface"}' \
  > /dev/null 2>&1 &
```

Running the **Reconnaissance** workflow in the **prompt-injection** skill to map attack surface...
