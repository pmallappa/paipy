...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the IndirectInjectionTesting workflow in the prompt-injection skill to test data sources"}' \
  > /dev/null 2>&1 &
```

Running the **IndirectInjectionTesting** workflow in the **prompt-injection** skill to test data sources...
