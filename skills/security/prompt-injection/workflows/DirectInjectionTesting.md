...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the DirectInjectionTesting workflow in the prompt-injection skill to test direct attacks"}' \
  > /dev/null 2>&1 &
```

Running the **DirectInjectionTesting** workflow in the **prompt-injection** skill to test direct attacks...
