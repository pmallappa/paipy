...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the AddCommand workflow in the create-cli skill to add CLI command"}' \
  > /dev/null 2>&1 &
```

Running the **AddCommand** workflow in the **create-cli** skill to add CLI command...
