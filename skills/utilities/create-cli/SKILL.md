...
`~/.claude/pai/user/skillcustomizations/create-cli/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:8888/notify \
     ...
     -d '{"message": "Running the WORKFLOWNAME workflow in the create-cli skill to ACTION"}' \
     > /dev/null 2>&1 &
   ```

2. **Output text notification**:
   ```
   Running the **WorkflowName** workflow in the **create-cli** skill to ACTION...
   ```
