...
`~/.claude/pai/user/skillcustomizations/pai-upgrade/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:8888/notify \
     ...
     -d '{"message": "Running the WORKFLOWNAME workflow in the pai-upgrade skill to ACTION"}' \
     > /dev/null 2>&1 &
   ```

2. **Output text notification**:
   ```
   Running the **WorkflowName** workflow in the **pai-upgrade** skill to ACTION...
   ```
...
| **Custom Source Agent** | Any USER-defined additional sources | USER/SKILLCUSTOMIZATIONS/pai-upgrade/ |
...
Run: bun ~/.claude/skills/utilities/pai-upgrade/tools/Anthropic.ts
...
**User Customizations** (`~/.claude/pai/user/skillcustomizations/pai-upgrade/`):
