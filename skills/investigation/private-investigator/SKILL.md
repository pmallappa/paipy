...
`~/.claude/pai/user/skillcustomizations/private-investigator/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:8888/notify \
     ...
     -d '{"message": "Running the WORKFLOWNAME workflow in the private-investigator skill to ACTION"}' \
     > /dev/null 2>&1 &
   ```

2. **Output text notification**:
   ```
   Running the **WorkflowName** workflow in the **private-investigator** skill to ACTION...
   ```

**This is not optional. Execute this curl command immediately upon skill invocation.**

# private-investigator - Ethical People Finding
